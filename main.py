...
for z in range(10):
    ...
    logName = './output/logs_' + str(z) + '.log'
    # Creating a csv file to store the generated dataset
    fileName = './output/data_' + str(z) + '.csv'
    ...
    while i < n + 1:
        ...
        print(D_rebar, D_index_min, D_index_max, D_index_max-D_index_min)
        ...
        for diff_d_tr in range(D_index_max-D_index_min):
            ...
            try:
                ...
                if numBarsSec<4:
                    print("NurBarSec<4 continue")
                    continue
            except:
                print("NumBarSec Error continue")
                continue
            ...
            coreY = coverY-parameters.cover-D_rebar/2-D_tr/2   # Distance from the section z-axis to the edge of the core concrete --  edge of the core concrete/inner edge of cover concrete
            coreZ = coverZ-parameters.cover-D_rebar/2-D_tr/2   # Distance from the section y-axis to the edge of the core concrete --  edge of the core concrete/inner edge of cover concrete
            ...
            w_h = (Width-2*D_rebar-2.5*parameters.cover)/Width
            d_h = (Depth-2*D_rebar-2.5*parameters.cover)/Depth
            ...
            d_y = Width - parameters.cover - D_rebar / 2-D_tr/2   # Distance from column outer edge to rebar in Y-axis
            d_z = Depth - parameters.cover - D_rebar / 2-D_tr/2   # Distance from column outer edge to rebar in Z-axis
            ...
            # cRd = 0.12
            # k_param = max(1 + (200 / D_rebar) ** (1 / 2), 2)
            # rho1 = max(As / (Width * Depth), 0.02)
            # sigma = max(P / Ac, 0.2 * fcd)
            # k1 = 0.15
            # vmin_param = 0.035 * k_param ** (3 / 2) * fck ** (1 / 2)

            # Shear provided by concrete in both directions
            # Vc = max((vmin_param + k1 * sigma),
            #          (cRd * k_param * (100 * rho1 * fck) ** (1 / 3) + k1 * sigma)) * Width * Depth

            # The number depends on the links arrangement
            if d_g == 2:
                N_arr_Z = 2
            elif spacingY > 150 and d_g >= 3:
                N_arr_Z = d_g
            elif spacingY > 75 and d_g >= 3:
            ...
            if w_g == 2:
                N_arr_Y = 2
            elif spacingZ > 150 and w_g >= 3:
                N_arr_Y = w_g
            elif spacingZ > 75 and w_g >= 3:
            ...
            spacingArray = [*range(int(round(s_min/0.05)), int(round(s_max/0.05))+1, 1)]
            spacingArray = [round(0.05 * x, 2) for x in spacingArray]
            print("Spacing array of ties")
            print(spacingArray)

            for spacing_tr in spacingArray:
                print("Tie spacing")
                print(spacing_tr)
                ...
                z_param_y = 0.9 * Depth  # Shear parameter (taken from the lecture, correct?)
                z_param_z = 0.9 * Width  # Shear parameter

                cotTheta_y = (spacing_tr*alpha_param*v1*(-fcd)*Width/(A_sw_y*fyd_tr)-1)**(1/2)
                if cotTheta_y >= 2.5:
                    cotTheta_y = 2.5
                elif cotTheta_y <= 1.0:
                    cotTheta_y = 1.0

                cotTheta_z = (spacing_tr*alpha_param*v1*(-fcd)*Depth/(A_sw_z*fyd_tr)-1)**(1/2)
                if cotTheta_z >= 2.5:
                    cotTheta_z = 2.5
                elif cotTheta_z <= 1.0:
                    cotTheta_z = 1.0
                ...
                list_P = Pmax * np.random.random(size=50)
                ...
                for v in range(len(list_P)):
                    ...
                    strain1 = './output/strain11_' +str(i)+'_'+str(0)+'_'+ str(v)+'.txt'
                    strain2 = './output/strain12_' +str(i)+'_'+str(0)+'_'+ str(v)+'.txt'
                    strain3 = './output/strain13_' +str(i)+'_'+str(0)+'_'+ str(v)+'.txt'
                    strain4 = './output/strain14_' +str(i)+'_'+str(0)+'_'+ str(v)+'.txt'
                    ...
                    if len(indices) >= 1:
                        ...
                        list_m = float(M_ult)*np.random.random(size=30)
                        ...
                        for m in range(len(list_m)):
                            ...
                            strain21 = './output/strain21_' + str(i)+'_'+str(v)+'_'+ str(m)+'.txt'
                            strain22 = './output/strain22_' + str(i)+'_'+str(v)+'_'+ str(m)+'.txt'
                            strain23 = './output/strain23_' + str(i)+'_'+str(v)+'_'+ str(m)+'.txt'
                            strain24 = './output/strain24_' + str(i)+'_'+str(v)+'_'+ str(m)+'.txt'
